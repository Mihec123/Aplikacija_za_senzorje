...
public class DraggableTreeView extends FrameLayout{
    ...
    public void setAdapter(TreeViewAdapter adapter){
        ...
        Log.d("123","adapter prezvel");
        adapter.setTreeViews();
        Log.d("123","adapter prezve2");
        notifyDataSetChanged();
        Log.d("123","adapter prezve3");
    }
    ...
    private void handleItemDrag(){
        Log.d("treeView","mobilna: " + (String) mobileNode.getData());
            ...
            if(outRect.contains(mLastEventX, mLastEventY))
            {
                ...
                if(mLastEventY < viewRect.top+view.getHeight()/2 && mLastEventX <= mDownX+dpToPx(40)){
                    ...
                    if(mPlaceholderCheck+mPlaceholderDelay < System.currentTimeMillis()) {
                        ...
                        if((maxLevels != -1 && maxLevels < level) ||(!lastNode.getParent().isSenzor() && !mobileNode.isSenzor())){
                ...
                if(mPlaceholderCheck+mPlaceholderDelay < System.currentTimeMillis()) {
                    ...
                    if((maxLevels != -1 && maxLevels < level) || (mobileNode.isSenzor() && lastNode.isSenzor() && mobileNode != lastNode) || (!mobileNode.isSenzor() && !lastNode.isSenzor() && lastNode != mobileNode)
                            ||(!mobileNode.isSenzor() && lastNode.isSenzor())){
                        Log.d("treeView", "zadnja: " + (String) lastNode.getData());
                        Log.d("treeView", "pogoj1: " + (maxLevels != -1 && maxLevels < level));
                        Log.d("treeView", "pogoj2: " +  (mobileNode.isSenzor() && lastNode.isSenzor() && mobileNode != lastNode));
                        Log.d("treeView", "pogoj3: " + (mobileNode != lastNode));
                        Log.d("treeView","mobilna: " + (String) mobileNode.getData());
